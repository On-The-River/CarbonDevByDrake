        ...
        <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8">
          <div class="tmplmsg-box">
            <div class="tmplmsg-preview">
              ...
              <div
                class="acea-row row-middle tmplmsg-preview-cont mb10"
                v-for="(item, index) in KeywordCheck"
                :key="item.kid"
              >
          ...
          <div class="tmplmsg-form">
            ...
            >
              ...
              <el-form-item :label="'已选择(' + KeywordCheck.length + '/5)'">
                <span v-if="KeywordCheck.length === 0"
                  >请先从上方选择关键词</span
                >
                <div v-else class="tmplmsg-form-check">
                  <div
                    class="tmplmsg-form-check-list mb10 acea-row row-between"
                    v-for="(item, index) in KeywordCheck"
                    :key="item.kid"
                    draggable="true"
  ...
  methods: {
    ...
    setTagsViewTitle() {
      ...
      const route = Object.assign({}, this.tempRoute, {
        title: `${title}-${this.$route.params.myId}`
      });
      this.$store.dispatch("tagsView/updateVisitedView", route);
    },
    handleChecked(val) {
      this.KeywordCheck = this.KeywordList.filter(item =>
        val.some(ele => ele == item.kid)
      );
    },
    onSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.loading = true;
          this.$route.params.myId === "0"
            ? myTempSaveApi(data)
                .then(res => {
                  this.$message.success("提交成功");
                  this.$router.push(
                    "/appSetting/publicRoutine/publicRoutineTemplate"
                  );
                  this.loading = false;
                })
                .catch(() => {
            ...
            : myTempUpdateApi({ id: this.$route.params.myId }, data)
                .then(res => {
                  this.$message.success("提交成功");
                  this.$router.push(
                    "/appSetting/publicRoutine/routineTemplate"
                  );
                  this.loading = false;
                })
                .catch(() => {
    ...
    getKeywordList() {
      ...
      getWeChatKeywordsByTidApi({ tid: this.$route.params.tid })
        .then(res => {
