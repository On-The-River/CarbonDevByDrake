          ...
          <el-row :gutter="24">
            ...
            <el-col :span="24">
              <el-form-item label="商品轮播图：" prop="imagelist">
                <div class="acea-row">
                  ...
                  <div
                    v-if="formValidate.imagelist.length < 10"
                    class="upLoadPicBox"
                    @click="modalPicTap('2')"
                  >
                    <div class="upLoad">
            ...
            <el-col :span="24">
              <el-form-item label="活动日期：" prop="timeVal">
                ...
                <span
                  >设置活动开启结束时间，用户可以在设置时间内发起参与拼团</span
                >
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="拼团时效(单位 小时)：" prop="effectiveTime">
                ...
                />
                <span
                  >用户发起拼团后开始计时，需在设置时间内邀请到规定好友人数参团，超过时效时间，则系统判定拼团失败，自动发起退款</span
                >
              </el-form-item>
            </el-col>
            <el-col :span="24">
            ...
            <el-col :span="24">
              <el-form-item label="购买数量限制：" prop="num">
                ...
                />
                <span
                  >活动时间内每个用户参与拼团的次数限制。例如设置为4，表示本次活动有效期内，每个用户最多可参与购买4次</span
                >
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="单次购买数量限制：" prop="onceNum">
                ...
                />
                <span
                  >用户参与拼团时，一次购买最大数量限制。例如设置为2，表示参与拼团时，用户一次购买数量最大可选择2个</span
                >
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="补齐人数：" prop="peopleNum">
                ...
                />
                <span
                  >当用户参与拼团后，成团时效内未成团情况下，设置补齐人数可虚拟成团。例如：成团人数为10人，补齐人数为4人，真实用户需要参与6人成团才可以在最后未成团时自动补齐虚拟人员</span
                >
              </el-form-item>
            </el-col>
            <el-col v-bind="grid2">
...
export default {
  ...
  methods: {
      ...
      combinationInfoApi({ id: id })
        .then(async res => {
          ...
          this.formValidate = {
            ...
            ficti: res.ficti,
            timeVal:
              res.startTimeStr && res.stopTimeStr
                ? [res.startTimeStr, res.stopTimeStr]
                : [],
            status: res.status,
            isShow: res.isShow,
            ...
            peopleNum: Math.floor(
              res.people - (res.virtualRation / 100) * res.people
            )
          };
          if (res.specType) {
            this.ManyAttrValue = res.attrValues;
    ...
    handleSubmit(name) {
      ...
      this.formValidate.virtualRation = Math.floor(
        ((this.formValidate.people - this.formValidate.peopleNum) /
