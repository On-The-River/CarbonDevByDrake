      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.ruleForm.region.forEach((el, index) => {
            el.title =
              el.city_ids.length > 0
                ? JSON.stringify(el.city_ids)
                : JSON.stringify([[0, 0]]);
          ...
          if (this.ruleForm.appoint) {
            this.ruleForm.free.forEach((el, index) => {
              ...
              el.cityId =
                el.city_ids.length > 0 ? el.city_ids.join(",") : "all";
            });
